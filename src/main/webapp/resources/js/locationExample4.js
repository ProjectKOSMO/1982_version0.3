...
function Test(){
   $.ajax({
      ...
      success:function(result){
         ...
         newbutton.innerHTML="로드뷰 보기";
         newbutton.setAttribute('class',"rvButton");
         ...
         try{
            ...
            for (var i = 0; i < result.length; i ++) {
                ...
                var iwContent = '<div id="infomation" style="padding:5px;">'+ result[i].shopName+'</div>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
...
//검색의 중심 좌표 이동 시 사용
   ...
   $("#searchPost").on("click",".main_right",function(){
      ...
      forlocation_y=$(this).children(".searchLocation_y").text();
      forlocation_x=$(this).children(".searchLocation_x").text();
      forshopname=$(this).children(".shopName").text();
